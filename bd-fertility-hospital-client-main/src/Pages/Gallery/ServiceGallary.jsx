...
const ServiceGallary = () => {
  ...
  useEffect(() => {
    fetch(
      "https://hospitalupdate.abinashfoundation.com/Gallery-route/getAllServiceGallery"
    )
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
    ...
    <div className="text-center">
      ...
      <div className=" gap-2 lg:px-20 md:px-16 lg:py-10 grid lg:grid-cols-4 md:grid-cols-3 grid-cols-1">
        {images.map((item, index) => (
          <div
            className="group flex justify-center text-center relative overflow-hidden rounded-md cursor-pointer"
            key={index}
          >
            <img
              className="lg:h-64 lg:w-72 rounded-lg object-cover ease-in-out duration-500 group-hover:rotate-6 group-hover:scale-125"
              src={item.img}
              alt=""
            />
